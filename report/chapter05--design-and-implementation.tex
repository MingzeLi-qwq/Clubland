...
\begin{itemize}
    ...
    \item \textbf{Django Summernote}: Integrating rich text editing (RTE) functionality
...
\subsubsection{Why Not Use a Full CMS?}
We considered using a full CMS solution (like WordPress) to handle rich text posts. 
However, the assignment requires a custom-coded implementation of core functionality. 
Instead of relying on a pre-built CMS, we chose \textbf{Django Summernote} as our RTE, 
allowing us to store media files and properly render rich text content in HTML blocks, 
while still meeting the requirement of writing our own implementation.
...
\begin{itemize}
    ...
    \item \textbf{CSS3}: Custom styles and responsive design
